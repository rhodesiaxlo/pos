...
@section('content')
    ...
    <div class="row page-title-row" id="dangqian" style="margin:5px;">
        ...
        @if(Gate::forUser(auth('admin')->user())->check('salesman.sales.create'))
            ...
            <table id="tags-table" class="table table-striped table-bordered">
                ...
                <tr>
                   {{-- <th data-sortable="false" class="hidden-sm"></th>--}}
                    ...
                    <th class="hidden-sm">业务员姓名</th>
                    <th class="hidden-sm">手机号</th>
                    <th class="hidden-md">性别</th>
                    <th class="hidden-md">业务员类型</th>
                    <th class="hidden-md">注册时间</th>
                    <th class="hidden-md">操作人</th>
...
<div class="modal-dialog modal-warning">
    <div class="modal-content">
        ...
        <div class="modal-footer">
            <form class="deleteForm" method="POST" action="/salesman/sales/destroy">
        ...
        <script>
            $(function () {
                ...
                var table = $("#tags-table").DataTable({
                    ...
                    ajax: {
                        url: "{{route('salesman.sales.index')}}",
                        type: 'POST',
                        data: function (d) {
                            d.cid = cid;
                    ...
                    columnDefs: [
                        {
                            ...
                            var row_edit = {{Gate::forUser(auth('admin')->user())->check('salesman.sales.edit') ? 1 : 0}};
                            var row_delete = {{Gate::forUser(auth('admin')->user())->check('salesman.sales.destroy') ? 1 :0}};
                            var row_push = {{Gate::forUser(auth('admin')->user())->check('salesman.sales.details') ? 1 : 0}};
                            var str = '';
                            //编辑
                            if (row_edit) {
                                str += '<a style="margin:3px;" href="/salesman/sales/edit/' + row['id'] + '" class="X-Small btn-xs text-success "><i class="fa fa-edit"></i> 编辑</a>';
                            }
                           /* //详情
                            if (row_push) {
                                str += '<a style="margin:3px;" href="/salesman/sales/' + row['id'] + '/details" class="X-Small btn-xs text-success "><i class="fa fa-edit"></i> 详情</a>';
                            }*/
                ...
                $("table").delegate('.delBtn', 'click', function () {
                    ...
                    $('.deleteForm').attr('action', '/salesman/sales/destroy/' + id);
                    $("#modal-delete").modal();
                });
